            ...
            <AnimatePresence>
                {showAdvice && recommendation && (
                    <motion.div
                        ...
                        className="mb-8 bg-gradient-to-br from-indigo-600 to-purple-700 text-white rounded-[2rem] overflow-hidden shadow-xl"
                    >
                        <div className="p-6">
                            {/* Header */}
                            <div className="flex items-start gap-4 mb-4">
                                ...
                                <div className="flex-1">
                                    <h4 className="font-black text-xl mb-1">{t.ui.recommendation}</h4>
                                    <p className="text-white/90 font-semibold text-lg">{recommendation.reason}</p>
                                </div>
                            </div>

                            {/* Context Badge */}
                            {recommendation.context && (
                                <div className="mb-4 px-4 py-2 bg-white/10 rounded-xl border border-white/20 inline-flex items-center gap-2">
                                    <Icon name="info" size={16} className="text-yellow-300" />
                                    <span className="text-sm font-medium text-white/90">{recommendation.context}</span>
                                </div>
                            )}
                            ...
                            {recommendation.why && recommendation.why.length > 0 && (
                                <div className="mb-4">
                                    <h5 className="text-xs font-black uppercase tracking-widest text-white/60 mb-2">
                                        {language === 'ru' ? 'ПОЧЕМУ ЭТО ЛУЧШИЙ ВЫБОР' : language === 'uk' ? 'ЧОМУ ЦЕ НАЙКРАЩИЙ ВИБІР' : 'WHY THIS IS THE BEST CHOICE'}
                                    </h5>
                                    <ul className="space-y-1">
                                        {recommendation.why.map((item, i) => (
                                            <li key={i} className="flex items-start gap-2 text-white/80">
                                                <Icon name="check-circle" size={16} className="mt-0.5 text-green-300 flex-shrink-0" />
                            ...
                            {recommendation.benefits && recommendation.benefits.length > 0 && (
                                <div className="mb-4">
                                    <h5 className="text-xs font-black uppercase tracking-widest text-white/60 mb-2">
                                        {language === 'ru' ? 'ПРЕИМУЩЕСТВА' : language === 'uk' ? 'ПЕРЕВАГИ' : 'BENEFITS'}
                                    </h5>
                                    <div className="flex flex-wrap gap-2">
                                        {recommendation.benefits.map((benefit, i) => (
                                            <span
                                                key={i}
                                                className="px-3 py-1 bg-white/10 rounded-lg text-xs font-bold text-white/90 border border-white/20"
                                            >
                                                ✓ {benefit}
                                            </span>
                                        ))}
                            ...
                            <button
                                ...
                                className="w-full sm:w-auto px-8 py-3 bg-white text-indigo-600 rounded-xl font-black text-base hover:bg-indigo-50 transition-all shadow-lg hover:shadow-xl hover:scale-[1.02] flex items-center justify-center gap-2"
                            >
